		...
		while(!pq.empty()){
			...
			for(int i=0;i<4;i++){
				...
				if(now.y + mv[i][1] < 0 || now.y + mv[i][1] >= m)
					continue;
				if(used[now.x + mv[i][0]][now.y + mv[i][1]] == false){
					used[now.x+ mv[i][0]][now.y+ mv[i][1]] = true;
					if(dp[now.x + mv[i][0]][now.y + mv[i][1]] > dp[now.x][now.y] + maze[now.x + mv[i][0]][now.y + mv[i][0]]){
						node tmp;
